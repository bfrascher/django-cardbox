...
class TestMTGCardParser:
    class TestMCIEngine:
        ...
        def test__parse_types_stats(self, text, types_e, power_e, toughness_e,
            ...
            types, po, to, lo, mana, cmc = MTGCardParser.MCIEngine._parse_types_stats(p)
            try:
                power = int(po)
            except ValueError:
                power = po
        ...
        #     card, dual_card, artist, rulings = MTGCardParser.MCIEngine.parse_card('', '')
        ...
        #     assert card.dual_type == MTGCard.DUAL_NONE

        #     assert card.legal_vintage == MTGCard.LEGALITY_LEGAL
        #     assert card.legal_legacy == MTGCard.LEGALITY_LEGAL
        #     assert card.legal_extended == MTGCard.LEGALITY_LEGAL
        #     assert card.legal_standard == MTGCard.LEGALITY_LEGAL
        #     assert card.legal_classic == MTGCard.LEGALITY_LEGAL
        #     assert card.legal_commander == MTGCard.LEGALITY_LEGAL
        #     assert card.legal_modern == MTGCard.LEGALITY_LEGAL
        ...
        def test_parse_card_jace_vryns_prodigy(self):
            card, dual_card, artist, rulings = MTGCardParser.MCIEngine.parse_card('ori', '60a')
            ...
            assert card.dual_type == MTGCard.DUAL_FLIP

            assert card.legal_vintage == MTGCard.LEGALITY_LEGAL
            assert card.legal_legacy == MTGCard.LEGALITY_LEGAL
            assert card.legal_extended == MTGCard.LEGALITY_NONE
            assert card.legal_standard == MTGCard.LEGALITY_LEGAL
            assert card.legal_classic == MTGCard.LEGALITY_NONE
            assert card.legal_commander == MTGCard.LEGALITY_LEGAL
            assert card.legal_modern == MTGCard.LEGALITY_LEGAL
...
−9: You get an emblem with "Whenever you cast a spell, target opponent puts the top five cards of his or her library into his or her graveyard."""
            ...
            assert dual_card.dual_type == MTGCard.DUAL_FLIP

            assert dual_card.legal_vintage == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_legacy == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_extended == MTGCard.LEGALITY_NONE
            assert dual_card.legal_standard == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_classic == MTGCard.LEGALITY_NONE
            assert dual_card.legal_commander == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_modern == MTGCard.LEGALITY_LEGAL
        ...
        def test_parse_card_alive_well(self):
            card, dual_card, artist, rulings = MTGCardParser.MCIEngine.parse_card('dgm', '121a')
...
Fuse (You may cast one or both halves of this card from your hand.)"""
            ...
            assert card.dual_type == MTGCard.DUAL_SPLIT

            assert card.legal_vintage == MTGCard.LEGALITY_LEGAL
            assert card.legal_legacy == MTGCard.LEGALITY_LEGAL
            assert card.legal_extended == MTGCard.LEGALITY_LEGAL
            assert card.legal_standard == MTGCard.LEGALITY_NONE
            assert card.legal_classic == MTGCard.LEGALITY_LEGAL
            assert card.legal_commander == MTGCard.LEGALITY_LEGAL
            assert card.legal_modern == MTGCard.LEGALITY_LEGAL
...
Fuse (You may cast one or both halves of this card from your hand.)"""
            ...
            assert dual_card.dual_type == MTGCard.DUAL_SPLIT

            assert dual_card.legal_vintage == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_legacy == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_extended == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_standard == MTGCard.LEGALITY_NONE
            assert dual_card.legal_classic == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_commander == MTGCard.LEGALITY_LEGAL
            assert dual_card.legal_modern == MTGCard.LEGALITY_LEGAL
...
{T}: Jagged-Scar Archers deals damage equal to its power to target creature with flying."""
        ...
        def test_parse_card_allosaurus_rider(self):
            card, dual_card, artist, rulings = MTGCardParser.MCIEngine.parse_card('ddaevg', '2')
